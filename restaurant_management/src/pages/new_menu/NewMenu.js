        ...
        <View style={ styles.container}>
            <View>
                <TextInput
                        ...
                        onChangeText={(nomMenu)=>{setnomMenu(nomMenu)}}
                />
            </View>
            <View style={{ flexDirection:'row',  justifyContent:'space-between',padding:horizontalScale(10) }}>
                <Text style={{ color:COLOR.color1,fontWeight:'bold' }}>Liste des plats</Text>
                <Text style={{ color:COLOR.color1,fontWeight:'bold' }}> {data.length}</Text>
            </View>
            <View>
                <FlatList
                    data={data}
                            ...
                            <View style={styles.card_counter}>
                                {
                                    ...
                                    (
                                        <TouchableOpacity onPress={()=>{removePlat(item.id)}}>
                                            <Ionicons name={ICON.valid} size={moderateScale(20)}/>
                                        </TouchableOpacity>
                                    ):
                                    (
                                        <TouchableOpacity onPress={()=>{addPlat(item.id)}}>
