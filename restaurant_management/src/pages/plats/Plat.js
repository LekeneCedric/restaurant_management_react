...
import { panierStore, userRoleStore } from "../../store";

export const Plats = ({ navigation, route }) => {
  const [produit, panier_produit, quantite_produit, addProduit, removeProduit] =
    panierStore((store) => [
      store.produit,
  ...
  useEffect(() => {
    let new_plats = dataTemp.filter((plat) => {
      return (
        plat.nom.toLowerCase().includes(search.toLowerCase()) ||
        plat.prixUnitaire.toString().includes(search.toLowerCase())
      );
    });
    setData(new_plats);
  }, [search]);
